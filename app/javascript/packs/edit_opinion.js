function adjustEnteredTextSize(event) {
  ...
  // 改行不可能なフォームの場合、その幅を、入力した文字列の幅に合わせて伸縮させる
...
document.addEventListener('input', adjustEnteredTextSize);

document.addEventListener('DOMContentLoaded', function() {
  const adjustSizeElements = document.querySelectorAll('.adjust-entered-text-width, .adjust-entered-text-size');
  adjustSizeElements.forEach(function(adjustSizeElement) {
    const virtualEventObject = { target: adjustSizeElement }
    adjustEnteredTextSize(virtualEventObject);
  });
});
